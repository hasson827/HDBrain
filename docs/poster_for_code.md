...
window.FIGURES = {
  _meta: {
    ...
    snapshot: "2026-07-20",
    warning: "Provisional. Re-run make_figures.py after any backend change.",
  },
  ...
  models: [
    { name: "Baseline (group median)", rmse: 296723, mae: 265658, r2: -1.178, mape: 40.34 },
    { name: "Linear Regression", rmse: 160181, mae: 128627, r2: 0.365, mape: 18.50 },
    { name: "Ridge (L2, a=1)", rmse: 160181, mae: 128627, r2: 0.365, mape: 18.50 },
    { name: "Lasso (L1, a=1)", rmse: 160181, mae: 128625, r2: 0.365, mape: 18.50 },
    { name: "Random Forest", rmse: 83928, mae: 66225, r2: 0.826, mape: 9.72 },
    { name: "XGBoost", rmse: 83674, mae: 67562, r2: 0.827, mape: 10.03, champion: true },
  ],
  champion_r2: "0.827",
  champion_rmse: "S$83,674",
  champion_mape: "10.03%",
  rf_r2: "0.826",
  rf_size: "16 GB",
  xgb_size: "8 MB",
  ...
  shap: [
    { feat: "year", val: 195886 },
    { feat: "floor_area_sqm", val: 65372 },
    { feat: "flat_type_code", val: 37405 },
    { feat: "dist_dhoby (CBD)", val: 33962 },
    { feat: "remaining_lease", val: 31511 },
    { feat: "storey_range_code", val: 14875 },
    { feat: "hawker_dist", val: 9432 },
    { feat: "mrt_dist", val: 8593 },
  ],

  /* ---- ablation (test R2 drop when the group is removed) ---- */
  ablation: [
    { grp: "year", drop: 0.2743 },
    { grp: "storey", drop: 0.0155 },
    { grp: "amenity distance", drop: 0.0034 },
    { grp: "floor area", drop: 0.0027 },
    { grp: "flat model", drop: 0.0022 },
  ],
  ablation_year_r2_after: "0.553",
...
**回填时记得把 `_meta.snapshot` 改成你这次跑的日期。**
...
- [ ] 序列化后的 Random Forest 模型文件实际大小 = __________
- [ ] 序列化后的 XGBoost 模型文件实际大小 = __________
- [ ] 序列化方式（joblib / pickle / `.json` / `.ubj`）？压缩了吗？ = __________

**如果这个数字站不住，请直接说，前端会把这条论点整条删掉。** 印一个撑不住的数字比不印更糟。
...
- [ ] `metrics.py` 里这个指标实际算的是均值（mean）还是中位数（median）？ = __________
- [ ] poster 上该印哪个名字？ = __________

（两者在有长尾误差时差别很大，评委很可能追问。）

---

### Q3 数据集口径的四个数字还成立吗？

- [ ] `n_transactions` = 981,450 —— 还是这个数？ = __________
- [ ] `n_features` = 29 —— 建模实际用的特征维度？ = __________
- [ ] `date_to` = `2026-07` —— 数据真覆盖到 2026 年 7 月？ = __________
- [ ] 时间切分点还是 **train ≤ 2023 / test ≥ 2024** 吗？ = __________
...
- [ ] 这个描述现在还准确吗？还是你已经补了 CPI 数据 / 换了处理方式？ = __________
- [ ] 如果补了，上面所有指标都会变，请务必在 §7 明确标注 = __________
...
- [ ] 有没有？ = __________
- [ ] 如果有，一句话说明它的价值，前端来判断值不值得挤进版面 = __________
...
- [ ] 预计冻结日期 = __________
- [ ] 在此之前还有哪些计划中的改动？ = __________

> **07-27/28 必须送印**。前端收到最终数字后还要重排版面（数字位数变化会挤动布局），请留至少 1 天缓冲。
...
- [ ] 五个阶段的划分对吗？有没有漏掉或者划错的？ = __________
- [ ] 上表里每段的内容描述准确吗？（这些是前端从代码文件名推的） = __________
- [ ] 主图文件名（对应 §5.3 表） = __________
- [ ] 技术标签带保留吗？要不要换？ = __________
...
- [ ] 从**算法展示**的角度，网站上哪几个页面最值得截图放上来？（比如 SHAP 单笔归因的瀑布图交互、分位数区间的可视化……） = __________
