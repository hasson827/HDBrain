...
export function initSt6() {
  ...
  root.innerHTML = `
    ...
    <p>Trained on transactions up to 2023, tested on 2024 onward &mdash; the model never sees the
       future it is judged on.</p>
    <label>Metric
      <select id="st6-metric">${METRICS.map((m) => `<option value="${m}">${m.toUpperCase()}</option>`).join("")}</select>
    </label>
    <div id="st6-chart" style="width: 100%; height: 360px;"></div>

    <h3>Quantile interval coverage (90% target)</h3>
    ...
    <h3>Feature ablation (test set impact of dropping each feature group)</h3>
    <table border="1" cellpadding="4">
    ...
    <h3>Why is the Baseline's R&sup2; negative?</h3>
    <p>The CPI file used to inflation-adjust prices ends in 2020-09; months after that are
       ...
       that upswing, so it systematically underpredicts 2024-2026 and its R&sup2; goes negative.</p>

    <h3>Global feature importance (mean |SHAP value|, top 10)</h3>
    <table border="1" cellpadding="4">
  ...
  const chart = window.echarts.init(document.getElementById("st6-chart"));
  const metricSelect = document.getElementById("st6-metric");
